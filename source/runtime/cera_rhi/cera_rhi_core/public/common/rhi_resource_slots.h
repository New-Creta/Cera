  ...
  namespace renderer
  {
    ...
    public:
      ...
      public:
         ...
         */
        Iterator(std::atomic_flag* flags, size_t capacity, size_t current)
            : m_flags(flags)
            , m_capacity(capacity)
            , m_current(current)
        {
        ...
        Iterator& operator++()
        {
          ...
          {
            ++m_current;
          } while (m_current < m_capacity && m_flags[m_current].test_and_set(std::memory_order_acquire));
          return *this;
        }
      ...
      public:
         ...
         * @param current The current index of the iterator.
         */
        ConstIterator(std::atomic_flag* flags, size_t capacity, size_t current)
            : m_flags(flags)
            , m_capacity(capacity)
            , m_current(current)
        {
    ...
    private:
      ...
      std::unique_ptr<std::atomic_flag[]> m_flags;
      size_t m_flag_capacity;
    };
  } // namespace renderer
} // namespace rex