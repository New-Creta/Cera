...
namespace cera
{
  namespace renderer
  {
    ...
    void DynamicDescriptorHeap::stage_descriptors(u32 rootParameterIndex, u32 offset, u32 numDescriptors, const D3D12_CPU_DESCRIPTOR_HANDLE srcDescriptor)
    {
      ...
      CERA_ASSERT_X(numDescriptors <= m_num_descriptors_per_heap && rootParameterIndex < s_max_descriptor_tables, "Cannot stage more than the maximum number of descriptor heaps");
      ...
      CERA_ASSERT_X((offset + numDescriptors) <= descriptor_table_cache.num_descriptors, "Number of descriptors exceeds the number of descriptors in the descriptor table.");
    ...
    {
      CERA_ASSERT_X(rootParameterIndex < s_max_descriptor_tables, "Root parameter index excees max descriptor tables");
    ...
    void DynamicDescriptorHeap::parse_root_signature(const std::shared_ptr<RootSignature>& rootSignature)
    {
      ...
      CERA_ASSERT_X(current_offset <= m_num_descriptors_per_heap, "The root signature requires more than the maximum number of descriptors per descriptor heap. Consider increasing the maximum number of descriptors per descriptor heap.");
    }
    ...
    void DynamicDescriptorHeap::commit_descriptor_tables(CommandList& commandList, std::function<void(ID3D12GraphicsCommandList*, u32, D3D12_GPU_DESCRIPTOR_HANDLE)> setFunc)
    {
      ...
      if(num_descriptors_to_commit > 0)
      {
        ...
        CERA_ASSERT_X(d3d_graphics_command_list != nullptr, "Invalid command list");
