...
namespace cera
{
    namespace internal
    {
        ...
        D3D12_UNORDERED_ACCESS_VIEW_DESC get_uav_desc(const D3D12_RESOURCE_DESC& res_desc, UINT mipSlice, UINT arraySlice = 0, UINT planeSlice = 0)
        {
    ...
    bool texture::resize(u32 width, u32 height, u32 depthOrArraySize )
    {
        // Resource can't be resized if it was never created in the first place.
        if (get_d3d_resource())
        {
            CD3DX12_RESOURCE_DESC res_desc(get_d3d_resource()->GetDesc());
            ...
            auto d3d_device = get_device()->get_d3d_device();
            ...
            get_d3d_resource()->SetName( get_resource_name().c_str() );

            resource_state_tracker::add_global_resource_state(get_d3d_resource().Get(), D3D12_RESOURCE_STATE_COMMON);
    ...
    D3D12_CPU_DESCRIPTOR_HANDLE texture::get_render_target_view() const
    {
        return m_render_target_view.get_descriptor_handle();
    }

    D3D12_CPU_DESCRIPTOR_HANDLE texture::get_depth_stencil_view() const
    {
        return m_depth_stencil_view.get_descriptor_handle();
    }

    D3D12_CPU_DESCRIPTOR_HANDLE texture::get_shader_resource_view() const
    {
        return m_shader_resource_view.get_descriptor_handle();
    }

    D3D12_CPU_DESCRIPTOR_HANDLE texture::get_unordered_access_view(u32 mip) const
    {
        return m_unordered_access_view.get_descriptor_handle(mip);
    }
    ...
    {
        return check_format_support(D3D12_FORMAT_SUPPORT1_TYPED_UNORDERED_ACCESS_VIEW) &&
               check_format_support(D3D12_FORMAT_SUPPORT2_UAV_TYPED_LOAD) &&
               check_format_support(D3D12_FORMAT_SUPPORT2_UAV_TYPED_STORE);
    }
    ...
    {
        DXGI_FORMAT format = get_d3d_resource_desc().Format;
    ...
    {
        auto format = get_d3d_resource_desc().Format;
    ...
    DXGI_FORMAT texture::get_typeless_format(DXGI_FORMAT format)
    {
    ...
    DXGI_FORMAT texture::get_SRGB_format(DXGI_FORMAT format)
    {
    ...
    DXGI_FORMAT texture::get_UAV_compatable_format(DXGI_FORMAT format)
    {
    ...
    void texture::create_views()
    {
        if (get_d3d_resource())
        {
            ...
            CD3DX12_RESOURCE_DESC desc(get_d3d_resource()->GetDesc());

            // Create RTV
            if ((desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET) != 0 && check_RTV_support())
            {
                m_render_target_view = device->allocate_descriptors(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
                d3d_device->CreateRenderTargetView(get_d3d_resource().Get(), nullptr,
                    m_render_target_view.get_descriptor_handle());
            }
            // Create DSV
            if ((desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL) != 0 && check_DSV_support())
            {
                m_depth_stencil_view = device->allocate_descriptors(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
                d3d_device->CreateDepthStencilView(get_d3d_resource().Get(), nullptr,
                    m_depth_stencil_view.get_descriptor_handle());
            }
            // Create SRV
            if ((desc.Flags & D3D12_RESOURCE_FLAG_DENY_SHADER_RESOURCE) == 0 && check_SRV_support())
            {
                m_shader_resource_view = device->allocate_descriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
                d3d_device->CreateShaderResourceView(get_d3d_resource().Get(), nullptr,
                    m_shader_resource_view.get_descriptor_handle());
            }
            ...
            {
                m_unordered_access_view = device->allocate_descriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, desc.MipLevels);
                for (int i = 0; i < desc.MipLevels; ++i)
                {
                    auto uavDesc = internal::get_uav_desc(desc, i);
                    d3d_device->CreateUnorderedAccessView(get_d3d_resource().Get(), nullptr, &uavDesc, m_unordered_access_view.get_descriptor_handle(i));
                }
            }
        }
    }
